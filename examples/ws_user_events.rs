//! Stream private user events (orders, fills, positions) over the WebSocket.
//!
//! ```text
...
//! reconnect itself on `Disconnected`; this example simply reports the drop.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // reconnect) in order, so the auth frame precedes the channel subscription.
    let mut sub = client.connect(vec![
        json!({ "type": "auth", "token": token }),
        json!({ "type": "subscribe", "channel": "user" }),
    ]);
