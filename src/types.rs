...
pub enum TimeInForce {
    ...
    /// Fill-or-kill.
    Fok,
    /// Post-only: the order is rejected if it would take liquidity (cross the
    /// book) on entry, guaranteeing it rests as a maker. Use for maker-only
    /// strategies that must never pay taker fees.
    ///
    /// Unlike the other variants this serializes as `PostOnly` (not uppercased)
    /// to match the exchange wire format.
