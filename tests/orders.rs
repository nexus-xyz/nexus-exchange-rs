...
#[test]
fn post_only_time_in_force_round_trips_as_pascalcase() {
    // The exchange wire format is `PostOnly` (PascalCase), not the `UPPERCASE`
    // form the other TimeInForce variants use. A round-trip locks that in so a
    // future `rename_all` change can't silently break maker-only orders.
    let json = serde_json::to_value(TimeInForce::PostOnly).unwrap();
    assert_eq!(json, serde_json::json!("PostOnly"));

    let parsed: TimeInForce = serde_json::from_value(serde_json::json!("PostOnly")).unwrap();
    assert_eq!(parsed, TimeInForce::PostOnly);
}

#[tokio::test]
async fn create_post_only_order_serializes_pascalcase_tif() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/orders"))
        .and(header("x-api-key", "nx_test"))
        // A post-only limit serializes `time_in_force: "PostOnly"` on the wire.
        .and(body_json(serde_json::json!({
            "market_id": "BTC-USDX-PERP", "side": "Sell", "order_type": "Limit",
            "price": "50000", "quantity": "0.1", "time_in_force": "PostOnly"
        })))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "order": {
                "id": "o1", "market_id": "BTC-USDX-PERP", "account_id": "0xabc", "side": "Sell",
                "order_type": "Limit", "price": "50000", "quantity": "0.1", "filled_qty": "0",
                "status": "Open", "time_in_force": "PostOnly", "created_at": 1, "updated_at": 1
            },
            "fills": []
        })))
        .mount(&server)
    ...
    let resp = authed(server.uri()).create_order(&order).await.unwrap();
    assert_eq!(resp.order.id, "o1");
    assert_eq!(resp.order.time_in_force, TimeInForce::PostOnly);
}
