...
#[tokio::test]
async fn fetch_market_adl_events_parses_nested_closures() {
    let server = MockServer::start().await;
    let body = serde_json::json!([{
        "market_id": "BTC-USDX-PERP", "target_account": "0xabc",
        "bankruptcy_price": "49000.0", "bad_debt_absorbed_by_fund": "1200.5",
        "counterparty_closures": [{
            "account_id": "0xdef", "position_closed": "0.5", "settlement_amount": "300.25"
        }],
        "sequence": 42, "timestamp": 1776033960368i64
    }]);
    Mock::given(method("GET"))
    ...
    let ev = client(server.uri())
        .fetch_market_adl_events("BTC-USDX-PERP", None)
        .await
        .unwrap();
    assert_eq!(ev.len(), 1);
    assert_eq!(ev[0].sequence, 42);
    assert_eq!(ev[0].counterparty_closures.len(), 1);
    assert_eq!(
        ev[0].counterparty_closures[0].settlement_amount.to_string(),
        "300.25"
    );
}

#[tokio::test]
async fn fetch_account_adl_history_sends_limit() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/account/0xabc/adl-history"))
    ...
    let ev = client(server.uri())
        .fetch_account_adl_history("0xabc", Some(5))
        .await
        .unwrap();
    assert!(ev.is_empty());
}
